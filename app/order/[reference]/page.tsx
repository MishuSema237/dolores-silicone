...
export default function OrderConfirmationPage() {
  ...
  const handleCopyReference = async () => {
    try {
      ...
      setCopied(true);
      toast.success("Referencia del pedido copiada!");
    ...
    <div className="w-full max-w-viewport mx-auto px-4 pt-4 md:pt-8 print:max-w-none">
      ...
      <div className="max-w-3xl mx-auto text-center print:text-left">
        ...
        <div className="mb-8 print:hidden">
          ...
          <h1 className="mb-4">¡Pedido Confirmado!</h1>
          <p className="text-lg text-gray-500 mb-2">
            Gracias por tu solicitud de pedido. La hemos recibido y te contactaremos
            pronto.
          </p>
        </div>
        ...
        <div className="hidden print:block mb-8 text-center">
          ...
          <h1 className="text-3xl font-bold mb-2">Recibo del Pedido</h1>
        ...
        <div className="bg-gray-100 p-8 border border-gray-300 text-left mb-8 print:bg-white print:border-none print:p-0">
          <h2 className="mb-6 text-center print:text-left print:text-2xl">Detalles del Pedido</h2>

          <div className="mb-6">
            <p className="font-semibold mb-2">Referencia del Pedido:</p>
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                aria-label="Copiar referencia del pedido"
              >
                {copied ? (
          ...
          <div className="border-t border-gray-300 pt-4 mb-6">
            <div className="flex justify-between text-gray-600 mb-2">
              <span>Subtotal</span>
              <span>{formatPrice(orderItems.reduce((acc: number, item: any) => acc + item.price * item.quantity, 0))}</span>
            </div>
            <div className="flex justify-between text-gray-600 mb-2">
              <span>Envío</span>
              <span>{formatPrice(total - orderItems.reduce((acc: number, item: any) => acc + item.price * item.quantity, 0))}</span>
            </div>
          ...
          <div className="bg-purple-50 p-4 border border-purple-200 rounded print:hidden">
            <p className="text-sm text-gray-700 mb-2">
              <strong>Próximos Pasos:</strong> Te contactaremos en{" "}
              {order.customer.email} dentro de 24 horas con los datos de pago. No se
              requiere ningún pago inmediato.
            </p>
            <a
              href={`${WHATSAPP.link}?text=${encodeURIComponent(`¡Hola! Acabo de hacer el pedido ${order.orderReference}. Tengo una pregunta.`)}`}
              target="_blank"
